            ...
            foreach (var line in Lines)
            {
                if (line is string[])
                {
                    for (int f = 0; f < colCountMax; f++)
                    {
                        ...
                        {
                            builder.Append(((string[])line)[f].PadRight(ColumnMaxLen[f] + Margin));
                            if (f < colCountMax - 1) builder.Append($"{ColumnChar} ");
                        }
                        else
                        {
                            builder.Append(new string(' ', ColumnMaxLen[f] + Margin));
                        }
                    }
                    builder.AppendLine();
                }
                ...
                {
                    builder.AppendLine((string)line);
                }
                else if (line is char)
                {
                    for (int f = 0; f < colCountMax; f++)
                    {
                        builder.Append(new string((char)line, ColumnMaxLen[f] + Margin));
                        if (f > 0) builder.Append(line);
                        if (f < colCountMax - 1) builder.Append(ColumnChar);
                    }
                    builder.AppendLine();
                }
