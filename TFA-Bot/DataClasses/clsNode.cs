    ...
    public class clsNode : ISpreadsheet<clsNode>
    {
        ...
        public uint HeightLowCount
        {
            ...
            set
            {
                ...
                if (!SyncMode)
                {
                    ...
                    {
                        Program.AlarmManager.Clear(AlarmHeightLow, $"CLEARED: {Name} height low. Now {LeaderHeight} {(DateTime.UtcNow - NodeGroup.Network.LastHeight.Value).ToMSDisplay()} late.");
                        AlarmHeightLow = null;
                    }
                    else if (_heightLowCount > 12 && _requestFailCount == 0 && AlarmHeightLow == null)
                    {
                        AlarmHeightLow = new clsAlarm(clsAlarm.enumAlarmType.Height, $"WARNING: {Name} height low ({LeaderHeight}). {(DateTime.UtcNow - NodeGroup.Network.LastHeight.Value).ToMSDisplay()} late and counting.", this);
        ...
        public void GetVersion(int timeout = 2000)
        {
            try
            {
                ...
                request.AddHeader("Content-type", "application/json");
                request.AddHeader("header", "value");
                request.AddJsonBody(new { jsonrpc = "2.0", id = 0, method = "properties" });
