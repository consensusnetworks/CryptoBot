    ...
    public class clsBotControl : IBotCommand
    {
        ...
        public void Run(MessageCreateEventArgs e)
        {
            ...
            if (commands.Length > 1)
            {
                ...
                {
                    case "reload":
                        e.Channel.SendMessageAsync(Program.Spreadsheet.LoadSettings());
                        break;
                    case "restart":
                    ...
                    case "debug":
                        ...
                        if (reg.Success)
                        {
                            var filename = Path.Combine(Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location), "mono_args.txt");
